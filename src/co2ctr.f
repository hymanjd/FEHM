...
c xhua error#6631 A non-optional actual argument must be present when invoking a procedure with an explicit interface
c         call mass_component_liq(ij,1)   this is old one
         call mass_component_liq(iflg,ij,1)  
c check for gas phase transition
c         call mass_component_gas(ij,1)   this is old one
         call mass_component_gas(iflg,ij,1)
c check for sc phase transition
c         call mass_component_sc(ij,1)    this is old one
...
c
c xhua error#6633 the type of actual argument differs from the type of the dummy argument      
c      character*80 dum1, dumb, dumc    this is the original one
